...
try {
  ...
  let post = {
      ...
      sexe: req.body.sexe // Nouveau champ ajouté
  };
  ...
  connection.query(query, function(err, rows) {
    if (err) {
      res.json({"Error" : true, "Message" : "Error 1 executing MySQL query"});
    } else {
      if (rows.length == 0) {
        ...
        connection.query(query, post, function(err, rows) {
          if (err) {
            res.json({"Error" : true, "Message" : "Error 2 executing MySQL query" + err});
          } else {
